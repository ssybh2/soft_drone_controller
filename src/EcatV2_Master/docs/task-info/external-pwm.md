...
This task requires our self-made servo control board.

This carrier board will read the EtherCAT module's command by UART at a baud rate of 921600 and output the corresponding
PWM signal.

UART message definition as follows:

```c
/* packet length 37 bytes */
struct servo_cmd_t {
    uint8_t header; // it will be ignored now
...
Since the baud rate is 921600 and each packet length is 37 bytes, in 8N1 mode, the theoretical maximum control frequency
would be ~2490Hz, with each frame having a ~0.40148ms latency.

Here are the checksum functions:
...
If you have this board, [this](https://github.com/AIM-EC/servo_controller) is the repo of servo control board firmware.

This control board should be connected to any ``UART`` port of your EtherCAT module.

#### Configuration items

* UART
    * This refers to the ID of the UART port, which is connected to the servo control board.
* Enabled Channel Count
    * This means how many channels of the external servo control board you want to use; this can be [1, 16].
* Initial Value
    * This means the initial high pulse width of pwm signal EtherCAT will send before receiving any command
...
```c
/* Message type: custom_msgs/msg/WriteExternalPWM */

uint16[] channels // the length of this array is the value of 'Enabled Channel Count'
```