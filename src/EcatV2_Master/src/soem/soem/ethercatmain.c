...
uint16 ecx_eeprom_waitnotbusyAP(ecx_contextt* context, uint16 aiadr, uint16* estat, int timeout)
{
    ...
    while (((wkc <= 0) || ((*estat & EC_ESTAT_BUSY) > 0)) && (osal_timer_is_expired(&timer) == FALSE));
        ...
        if (context->grouplist[group].blockLRW)
        {
            ...
            if (context->grouplist[group].Ibytes)
            {
                ...
                do
                {
                    ...
                    {
                        sublength = (uint16)(context->grouplist[group].IOsegment[currentsegment++] - context->grouplist[
                            group].Ioffset);
                    }
                    else
                    {
