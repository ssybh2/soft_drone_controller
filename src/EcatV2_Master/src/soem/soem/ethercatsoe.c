                ...
                if (wkc > 0) /* succeeded to read slave response ? */
                {
                    ...
                    else
                    {
                        if (((aSoEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_SOE) &&
                            ...
                            (aSoEp->error == 1))
                        {
                            mp = (uint8*)&MbxIn + (etohs(aSoEp->MbxHeader.length) + sizeof(ec_mbxheadert) - sizeof(
                                uint16));
                            errorcode = (uint16*)mp;
                            ecx_SoEerror(context, slave, idn, *errorcode);
                        }
...
int ecx_readIDNmap(ecx_contextt* context, uint16 slave, uint32* Osize, uint32* Isize)
{
    ...
    *Osize = 0;
    for (driveNr = 0; driveNr < EC_SOE_MAX_DRIVES; driveNr++)
    {
        ...
        wkc = ecx_SoEread(context, slave, driveNr, EC_SOE_VALUE_B, EC_IDN_MDTCONFIG, &psize, &SoEmapping,
                          EC_TIMEOUTRXM);
        if ((wkc > 0) && (psize >= 4) && ((entries = etohs(SoEmapping.currentlength) / 2) > 0) && (entries <=
            EC_SOE_MAXMAPPING))
        {
            ...
            *Osize += 16;
            for (itemcount = 0; itemcount < entries; itemcount++)
            {
                ...
                wkc = ecx_SoEread(context, slave, driveNr, EC_SOE_ATTRIBUTE_B, SoEmapping.idn[itemcount], &psize,
                                  &SoEattribute, EC_TIMEOUTRXM);
